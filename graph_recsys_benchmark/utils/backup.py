...
def update_pea_graph_input(dataset_args, train_args, dataset):
    if dataset_args['dataset'] == "Movielens":
        ...
        if dataset_args['name'] == "25m":
            ...
            meta_path_edge_indicis_13 = [torch.flip(user2item_edge_index, dims=[0]), torch.flip(tag2user_edge_index, dims=[0]), tag2user_edge_index]
            meta_path_edge_indicis_14 = [genome_tag2item_edge_index, torch.flip(tag2item_edge_index, dims=[0]), tag2user_edge_index]
            meta_path_edge_indicis_15 = [genre2item_edge_index, torch.flip(tag2item_edge_index, dims=[0]), tag2user_edge_index]
            meta_path_edge_indicis_16 = [genome_tag2item_edge_index, torch.flip(tag2item_edge_index, dims=[0]), tag2item_edge_index]
            meta_path_edge_indicis_17 = [genre2item_edge_index, torch.flip(tag2item_edge_index, dims=[0]), tag2item_edge_index]
