...
from graph_recsys_benchmark.models import HeRecRecsysModel
...
class HeRecRecsysModel(HeRecRecsysModel):
...
class HeRecSolver(BaseSolver):
    def __init__(self, model_class, dataset_args, model_args, train_args):
        super(HeRecSolver, self).__init__(model_class, dataset_args, model_args, train_args)
            ...
            if start_run <= self.train_args['runs']:
                for run in range(start_run, self.train_args['runs'] + 1):
                    ...
                    if self.dataset_args['dataset'] == "Movielens":
                        edge_index_dict = {
                            ('genre', 'as the genre of', 'iid'): torch.from_numpy(
                                dataset.edge_index_nps['genre2item']).long().to(self.train_args['device']),
                            ('iid', 'has been watched by', 'uid'): torch.from_numpy(
                                np.flip(dataset.edge_index_nps['user2item'], 0).copy()).long().to(
                                self.train_args['device']),
                            ('uid', 'has watched', 'iid'): torch.from_numpy(
                                dataset.edge_index_nps['user2item']).long().to(self.train_args['device']),
                            ('iid', 'has the genre', 'genre'): torch.from_numpy(
                                np.flip(dataset.edge_index_nps['genre2item'], 0).copy()).long().to(
                                self.train_args['device']),
                        }
                        metapath_1 = [
                            ('uid', 'has watched', 'iid'),
                    ...
                    for metapath_idx, metapath in enumerate(metapaths):
                        ...
                        weights_file = os.path.join(weights_path, 'random_walk_{}_meta_{}.pkl'.format(self.model_args['walks_per_node'], metapath_idx))
                        if os.path.isfile(weights_file):
                            ...
                            weightpath = os.path.join(weights_path, 'random_walk_{}_meta_{}.pkl'.format(self.model_args['walks_per_node'], metapath_idx))
                        ...
                        with torch.no_grad():
                            ...
                            embeddings.append(torch.tensor(random_walk_model.embedding.weight, requires_grad=False))

                    self.model_args['embeddings'] = embeddings
                    self.model_args['num_iids'] = dataset.num_iids
                    self.model_args['num_uids'] = dataset.num_uids
                    self.model_args['acc_uids'] = dataset.type_accs['uid']
                    self.model_args['acc_iids'] = dataset.type_accs['iid']
...
if __name__ == '__main__':
    solver = HeRecSolver(HeRecRecsysModel, dataset_args, model_args, train_args)
    solver.run()
