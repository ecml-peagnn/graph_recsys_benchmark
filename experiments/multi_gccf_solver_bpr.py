...
sys.path.append('..')
from graph_recsys_benchmark.models import MultiGCCFRecsysModel
...
model_args = {
    ...
    'emb_dim': args.emb_dim, 'hidden_size': args.hidden_size, 'repr_dim': args.repr_dim,
...
class MultiGCCFRecsysModel(MultiGCCFRecsysModel):
    def update_graph_input(self, dataset):
        ...
        user2item_edge_index = dataset.edge_index_nps['user2item']
        user2item_edge_index = torch.from_numpy(user2item_edge_index).long().to(train_args['device'])
        item2user_edge_index = np.flip(dataset.edge_index_nps['user2item'], 0).copy()
        item2user_edge_index = torch.from_numpy(item2user_edge_index).long().to(train_args['device'])
        return edge_index, user2item_edge_index, item2user_edge_index


if __name__ == '__main__':
    solver = BaseSolver(MultiGCCFRecsysModel, dataset_args, model_args, train_args)
    solver.run()
